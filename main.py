...
def main():
    ...
    res_list_str = (to_str([PlayerCards(hands_force[j][2], hands_force[j][3])
                            for j in range(len(hands_force))])).split(" ")[1:]
...
def force_definition(hands_force: List[int]) -> List[int] and List[PlayerCards]:
